...
function renderDetail(row) {
  if (!row) {
    els.detailBox.innerHTML = '<p class="small">Select “View details” to inspect content metadata.</p>';
    return;
  }
...
async function renderSaved() {
  if (!state.currentUser) {
    els.savedList.innerHTML = '<p class="small">Sign in to save content.</p>';
...
async function renderContinueWatching() {
  ...
  els.continueBox.innerHTML = row
    ? `<article class="item"><strong>${row.content.title}</strong><p class="small">${row.progress.progressSeconds}s tracked • updated ${new Date(row.progress.updatedAt).toLocaleString()}</p></article>`
